	...
	McGrid.prototype.setList = function(arrList, nTotCnt, isSort) {
		...
		for(var j=0; j < arrList.length; j++) {
			...
			for(var k = 0; k < this.arrHeader.length; k++) {
				...
				var fieldType = this.arrHeader[k].type || 'string';
				var colDiv = document.createElement('div');
				...
				if (this.isObj){
					if (fieldType === 'number') {
						colDiv.innerHTML = Number(arrList[j][this.arrHeader[k].id]).toLocaleString('ko');
						colDiv.title = Number(arrList[j][this.arrHeader[k].id]).toLocaleString('ko');
					} else if (fieldType === 'date') {
						strDate = arrList[j][this.arrHeader[k].id];
						strDate = strDate.substring(2,4) + '.' + strDate.substring(4,6) + '.' + strDate.substring(6,8);
						colDiv.innerHTML = strDate;
						colDiv.title = strDate;
					...
					if (fieldType === 'number') {
						colDiv.innerHTML = Number(arrList[j][k]).toLocaleString('ko');
						colDiv.title = Number(arrList[j][k]).toLocaleString('ko');
					} else if (fieldType === 'date') {
						strDate = arrList[j][k];
						strDate = strDate.substring(2,4) + '.' + strDate.substring(4,6) + '.' + strDate.substring(6,8);
						colDiv.innerHTML = strDate;
			...
			document.querySelectorAll('#'+this.target +'.mcGrid-box > .pagination a')[i].addEventListener('click', function(event) {
				...
				if(event.target.innerHTML == '«'){ // 앞으로
					if(currentPageCnt > 1){
						...
						endNum = _self.nCurrentPage * _self.nMaxListCnt;
